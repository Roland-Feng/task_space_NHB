...
class bipartite_sbm():
    ...
    def fit_mcmc(self,overlap = False, n_init = 1, verbose=False, epsilon=1e-3):
        '''
        Fit the sbm to the word-document network.
        - overlap, bool (default: False). Overlapping or Non-overlapping groups.
            Overlapping not implemented yet
        ...
        if g is None:
            ...
            for i_n_init in range(n_init):
                ...
                tempt = 0
                while abs(delta) > epsilon:
                    delta = state_tmp.multiflip_mcmc_sweep(niter=100, beta=np.inf)[0]
                    #print(delta)
    ...
    def fit_equilibrate(self,overlap = False, n_init = 1, verbose=False, mcmc_niter=100):
        '''
        Fit the sbm to the word-document network.
        - overlap, bool (default: False). Overlapping or Non-overlapping groups.
            Overlapping not implemented yet
        ...
        if g is None:
            ...
            for i_n_init in range(n_init):
                ...
                gt.mcmc_equilibrate(state_tmp, mcmc_args=dict(niter=mcmc_niter), callback=print_iterative)
