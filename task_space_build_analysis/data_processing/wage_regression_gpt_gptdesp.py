...
def bert_skill_embedding_regression_gptdesp(job_skill_dataset, data_path_save, device, data_label):
    ...
    save_obj(occupation_skill_embedding,f'gptdesp_regression_job_skill_embedding', data_path_save + f'jobs/{data_label}/')
    return


def bert_task_description_embedding_regression_gptdesp(community_unweighted_level, data_path_save, device, level, data_label, task_description):
    from sentence_transformers import SentenceTransformer
    ...
    for t, tdesp in tqdm(task_description.items()):
        if len(community_unweighted_level[str(t)]) > 0:
            task_description_embedding[t] = model.encode(tdesp, convert_to_tensor=True)

    save_obj(task_description_embedding, f'gptdesp_regression_task_description_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    return


def occupation_task_distance_matrix_regression_gptdesp(occupation_embedding_dict, community_unweighted_level, community_list_std, data_path_save, level, data_label):
    
    import torch
    from sentence_transformers import util

    task_description_embedding = load_obj(f'gptdesp_regression_task_description_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    
    #community_embedding_average = torch.stack([torch.mean(torch.stack([task_description_embedding[c]]), 0) for c in community_list_std if len(community_unweighted_level[c]) > 0])
    community_embedding_average = torch.stack([task_description_embedding[int(c)] for c in community_list_std if len(community_unweighted_level[c]) > 0])
    ...
    save_obj(occupation_task_similarity_matrix, f'gptdesp_regression_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
...
def build_regression_dataframe_gptdesp(data_path_save,level,community_list_std,community_unweighted_level,skill_similarity_threshold,skill_length_threshold,so_year_list, task_salary_name, data_label, density_user_label, salary_type = 'mean'):
    ...
    occupation_task_similarity_matrix = load_obj(f'gptdesp_regression_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
