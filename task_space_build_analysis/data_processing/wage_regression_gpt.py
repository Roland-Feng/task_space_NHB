...
def bert_skill_embedding_regression(job_skill_dataset, data_path_save, device, data_label):
    ...
    save_obj(occupation_skill_embedding,f'regression_job_skill_embedding', data_path_save + f'jobs/{data_label}/')
    return


def bert_tag_embedding_regression(community_unweighted_level, data_path_save, device, level, data_label):
    from sentence_transformers import SentenceTransformer
    ...
    save_obj(tag_embedding, f'regression_tag_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    return


def occupation_task_distance_matrix_regression(occupation_embedding_dict, community_unweighted_level, community_list_std, data_path_save, level, tag_count_dict, tag_count_bool, data_label):
    ...
    tag_embedding = load_obj(f'regression_tag_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    ...
    save_obj(occupation_task_similarity_matrix, f'regression_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
...
def build_regression_dataframe(data_path_save,level,community_list_std,community_unweighted_level,skill_similarity_threshold,skill_length_threshold,so_year_list, task_salary_name, data_label, density_user_label, salary_type = 'mean'):
    ...
    occupation_task_similarity_matrix = load_obj(f'regression_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
...
def build_regression_dataframe_with_city(data_path_save,level,tag_core_bool,community_list_std,community_unweighted_level,skill_similarity_threshold,skill_length_threshold,so_year_list, task_salary_name, data_label, salary_type = 'mean'):
    ...
    ##! relatedness
    print('get relatedness of tasks')
    cc_pmi = load_obj(f'cc_pmi_half_user_2008_2023_level_{level}', data_path_save + f'vote_regression_together/user_task_collection/')
    job_task_relatedness_average = get_job_task_relatedness(cc_pmi, job_task_dict, C_dict)
    occupation_index_set = occupation_index_set.intersection(job_task_relatedness_average.keys())
    ...
    ##! task ubiquity
    print('get task ubiquity')
    task_ubiquity_so_user = get_task_ubiquity_in_so_user(data_path_save, so_year_list, tag_community_dict, tag_core_bool)
    job_task_ubiquity_average = get_job_task_ubiquity_average(job_task_dict, occupation_index_set, task_ubiquity_so_user)
    ...
    ##! job city
    job_city_regression_dict = load_obj(f'regression_job_city_dict', data_path_save + f'jobs/{data_label}/')
    job_city_keys = set([j for j, jc in job_city_regression_dict.items() if jc != 'not_sure' and jc != 'Remote'])

    occupation_index = list(occupation_index_set.intersection(job_city_keys))
    salary_list = [salary_dict[oi] for oi in occupation_index]
    task_length_list = [len(job_task_dict[oi]) for oi in occupation_index]
    task_relatedness_list = [job_task_relatedness_average[oi] for oi in occupation_index]
    job_task_average_salary_list = [job_average_task_salary[oi] for oi in occupation_index]
    job_task_ubiquity_average_list = [job_task_ubiquity_average[oi] for oi in occupation_index]
    job_year_list = [job_year_regression_dict[oi] for oi in occupation_index]
    job_city_list = [job_city_regression_dict[oi] for oi in occupation_index]

    print('build dataframe')
    df_with_city = pd.DataFrame.from_dict({'occupation_index': occupation_index, 
                                'salary': salary_list, 
                                'job_task_length': task_length_list,
                                'job_task_relatedness': task_relatedness_list,
                                'job_task_salary_average': job_task_average_salary_list,
