...
def bert_skill_embedding_threshold(job_skill_dataset, data_path_save, device, data_label):
    ...
    save_obj(occupation_skill_embedding,f'threshold_job_skill_embedding', data_path_save + f'jobs/{data_label}/')
    return


def bert_tag_embedding_threshold(community_unweighted_level, data_path_save, device, level, data_label):
    from sentence_transformers import SentenceTransformer
    ...
    save_obj(tag_embedding, f'threshold_tag_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    return


def occupation_task_distance_matrix_threshold(occupation_embedding_dict, community_unweighted_level, community_list_std, data_path_save, level, tag_count_dict, tag_count_bool, data_label):
    ...
    tag_embedding = load_obj(f'threshold_tag_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    ...
    save_obj(occupation_task_similarity_matrix, f'threshold_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
...
##! build user set
def get_job_community_set_threshold(occupation_task_similarity_matrix ,community_list_std, community_list_std_no_empty, skill_similarity_threshold = 0):
...
def predict_user_community_ignore_target_threshold(cc_pmi_matrix, user_community_vector_binary, community_list_std, user_target):
    C_dict = {c:i for i,c in enumerate(community_list_std)}
    user_prediction = {}
...
##! examine prediction
def examine_prediction_threshold(user_prediction, user_target, user_source, community_list_std):
