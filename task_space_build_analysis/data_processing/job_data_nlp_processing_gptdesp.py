...
def bert_skill_embedding_gptdesp(job_skill_dataset, data_path_save, device, data_label):
    ...
    save_obj(occupation_skill_embedding,f'gptdesp_job_skill_embedding', data_path_save + f'jobs/{data_label}/')
    return


def bert_task_description_embedding_gptdesp(community_unweighted_level, data_path_save, device, level, data_label, task_description):
    from sentence_transformers import SentenceTransformer
    ...
    save_obj(task_description_embedding, f'gptdesp_task_description_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    return


def occupation_task_distance_matrix_gptdesp(occupation_embedding_dict, community_unweighted_level, community_list_std, data_path_save, level, data_label):
    
    import torch
    from sentence_transformers import util

    task_description_embedding = load_obj(f'gptdesp_task_description_embedding_level_{level}', data_path_save + f'jobs/{data_label}/')
    ...
    save_obj(occupation_task_similarity_matrix, f'gptdesp_job_task_similarity_matrix_level_{level}', data_path_save + f'jobs/{data_label}/')
...
##! build user set
def get_job_community_set_gptdesp(occupation_task_similarity_matrix ,community_list_std, community_list_std_no_empty, skill_similarity_threshold = 0):
...
def predict_user_community_ignore_target_gptdesp(cc_pmi_matrix, user_community_vector_binary, community_list_std, user_target):
    C_dict = {c:i for i,c in enumerate(community_list_std)}
    user_prediction = {}
...
##! examine prediction
def examine_prediction_gptdesp(user_prediction, user_target, user_source, community_list_std):
